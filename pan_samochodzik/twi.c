...
extern uint8_t l_motor_pwm;
extern uint8_t r_motor_pwm;
extern uint8_t  angle_val;
extern volatile uint8_t echo_val;
extern volatile unsigned char flag_reg;
...
void answer(){
	...
	switch(TWI_buf[0]){
		case GET_ECHO_DISTANCE:
			TWI_Start_Transceiver_With_Data(&echo_val,1 );

			break;


		case SET_L_MOTOR:
			if(TWI_buf[1]&(1<<DIR_BIT))
				SET_L_DIR_ON;
			else
				SET_L_DIR_OFF;

			l_motor_pwm = ( TWI_buf[1] >> (1 + DIR_BIT));
		...
		case SET_R_MOTOR:
			...
			r_motor_pwm = ( TWI_buf[1] >> (1 + DIR_BIT));
			TWI_statusReg.RxDataInBuf = FALSE;
			break;
		case SET_ANGLE:
			angle_val = ( TWI_buf[1] >> (1 + DIR_BIT));
			TWI_statusReg.RxDataInBuf = FALSE;
			break;
